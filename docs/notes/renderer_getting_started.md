...
- **texturing** (e.g interpolation of vertex RGB colors or interpolation of vertex UV coordinates followed by sampling from a texture map or a custom uv map(interpolation uses barycentric coordinates output from rasterization))
...
| NormalShader || :heavy_check_mark: |||||  :heavy_check_mark:|
| UVsCorrespondenceShader|| :heavy_check_mark: |||||  :heavy_check_mark:|
